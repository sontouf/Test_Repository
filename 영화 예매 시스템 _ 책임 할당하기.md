...
안타깝게도 클래스를 분리한 후에 새로운 문제가 나타났다 수정 전에는 Movie 와 협력하는 클래스는 DiscountCondition 하나 뿐이었다. 그러나 수정 후에  Movie 인스턴스는 SequenceCondition 과 PeriodCOndition 이라는 두 개의 다른 클래스의 서루 다른 클래스의 인스턴스 모두와 협력할 수 있어야 햔한다.
